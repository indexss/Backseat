...
public class TrackRepositoryWithBagRelationshipsImpl implements TrackRepositoryWithBagRelationships {
    ...
    public Optional<Track> fetchBagRelationships(Optional<Track> track) {
        return track.map(this::fetchArtists).map(this::fetchFolderEntries).map(this::fetchWantToListenListEntries);
    }
    ...
    Track fetchFolderEntries(Track result) {
        return entityManager
            .createQuery("select track from Track track left join fetch track.folderEntries where track is :track", Track.class)
            .setParameter("track", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }
    ...
    Track fetchWantToListenListEntries(Track result) {
        return entityManager
            .createQuery("select track from Track track left join fetch track.wantToListenListEntries where track is :track", Track.class)
            .setParameter("track", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }
